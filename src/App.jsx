...
function App() {
  ...
  const onSubmit = (updatedFilters) => setFilters(updatedFilters);

  const searchSet = getSearchSet(filters.search);

  const filteredKsbs = ksbs.filter(
    (ksb) =>
      searchSet.has(ksb.Code) &&
      filters.ksbs[ksb.Code.charAt(0)] &&
      filters.assessmentMethods[ksb.AssessmentMethod]
  );
